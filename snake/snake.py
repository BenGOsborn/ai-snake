...
class Snake:
    def __init__(self, width, height):
        ...
        for i in range(height):
            for j in range(width):
                if (i != self.snake[0][0] or j != self.snake[0][1]) and (i != self.food[0] or j != self.food[1]):
    ...
    def game_over(self):
        return self.terminated or len(self.available) == 0
    ...
    def update_state(self, key=None):
        ...
        new_pos = (self.snake[0] + self.dir[0], self.snake[1] + self.dir[1])
        self.snake.insert(0, new_pos)
        tail = self.snake.pop(-1)

        if new_pos in self.available:
            self.available.remove(new_pos)
        self.available.add(tail)
